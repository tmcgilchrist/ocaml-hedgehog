...
let test_tree () =
  group "Tree" (fun () ->
      ...
      check "map preserves structure" (fun () ->
          let t =
            Hedgehog.Tree.Node
              (1, fun () -> Seq.Cons (Hedgehog.Tree.singleton 2, Seq.empty))
          in
      ...
      check "bind works" (fun () ->
          ...
          let t' =
            Hedgehog.Tree.bind t (fun x -> Hedgehog.Tree.singleton (x + 1))
          in
      ...
      check "mzip pairs values" (fun () ->
          let t1 =
            Hedgehog.Tree.Node
              (1, fun () -> Seq.Cons (Hedgehog.Tree.singleton 0, Seq.empty))
          in
          let t2 =
            Hedgehog.Tree.Node
              (10, fun () -> Seq.Cons (Hedgehog.Tree.singleton 5, Seq.empty))
          in
      ...
      check "mzip generates shrinks from both sides" (fun () ->
          ...
          let child_vals =
            List.of_seq
              (Seq.map Hedgehog.Tree.value (Hedgehog.Tree.children tz))
          in
          ...
          let trees =
            [
              Hedgehog.Tree.Node
                (1, fun () -> Seq.Cons (Hedgehog.Tree.singleton 0, Seq.empty));
              Hedgehog.Tree.Node
                (2, fun () -> Seq.Cons (Hedgehog.Tree.singleton 0, Seq.empty));
            ]
          in
      ...
      check "interleave shrinks towards empty" (fun () ->
          ...
          let result = Hedgehog.Tree.interleave trees in
          let first_child =
            match Hedgehog.Tree.children result () with
      ...
      check "prune 0 removes all children" (fun () ->
          ...
          match Hedgehog.Tree.children t' () with Seq.Nil -> true | _ -> false);
                ...
                fun () ->
                  Seq.Cons
                    ( Hedgehog.Tree.Node
                        ( 2,
                          fun () ->
                            Seq.Cons (Hedgehog.Tree.singleton 3, Seq.empty) ),
                      Seq.empty ) )
          in
      ...
      check "filter keeps root, removes non-matching children" (fun () ->
          let t =
            Hedgehog.Tree.Node
              ( 10,
                fun () ->
                  Seq.Cons
                    ( Hedgehog.Tree.singleton 5,
                      fun () -> Seq.Cons (Hedgehog.Tree.singleton 15, Seq.empty)
                    ) )
          in
          ...
          let child_vals =
            List.of_seq
              (Seq.map Hedgehog.Tree.value (Hedgehog.Tree.children t'))
          in
          child_vals = [ 5 ]);

      check "map_option returns None when root fails" (fun () ->
          let t = Hedgehog.Tree.singleton 5 in
          Hedgehog.Tree.map_option (fun x -> if x > 10 then Some x else None) t
          = None);

      check "map_option returns Some when root passes" (fun () ->
          let t = Hedgehog.Tree.singleton 15 in
          match
            Hedgehog.Tree.map_option
              (fun x -> if x > 10 then Some (x * 2) else None)
              t
          with
...
let test_gen () =
  group "Gen" (fun () ->
      ...
      check "bind chains generators" (fun () ->
          let g =
            Hedgehog.Gen.bind (Hedgehog.Gen.return 10) (fun x ->
                Hedgehog.Gen.return (x + 5))
          in
          match g 30 seed with
      ...
      check "and+ syntax works" (fun () ->
          let open Hedgehog.Gen in
          let g =
            let+ x = return 10 and+ y = return 20 in
            x + y
          in
          match g 30 seed with
          | Some t -> Hedgehog.Tree.value t = 30
      ...
      check "integral generates in range" (fun () ->
          ...
          let results =
            Array.init 50 (fun i ->
                let s = Hedgehog.Seed.from (Int64.of_int (i + 1)) in
                match g 30 s with Some t -> Hedgehog.Tree.value t | None -> -1)
          in
          Array.for_all (fun v -> v >= 10 && v <= 20) results);
      ...
      check "integral shrinks towards origin" (fun () ->
          ...
          | Some t -> (
              ...
              match Hedgehog.Tree.children t () with
              | Seq.Nil ->
                  v = 0 (* Only no shrinks if value is already origin *)
              | Seq.Cons (child, _) ->
      ...
      check "string generates correct length" (fun () ->
          let g =
            Hedgehog.Gen.string (Hedgehog.Range.constant 5 5) Hedgehog.Gen.alpha
          in
          match g 30 seed with
          | Some t ->
      ...
      check "list has shrinks towards shorter" (fun () ->
          let g =
            Hedgehog.Gen.list
              (Hedgehog.Range.constant 3 3)
              (Hedgehog.Gen.integral (Hedgehog.Range.constant 0 10))
          in
          match g 30 seed with
          | Some t -> (
              let xs = Hedgehog.Tree.value t in
      ...
      check "filter only generates passing values" (fun () ->
          let g =
            Hedgehog.Gen.filter
              (fun x -> x > 50)
              (Hedgehog.Gen.integral (Hedgehog.Range.constant 0 100))
          in
          let results =
            Array.init 20 (fun i ->
      ...
      check "resize overrides size" (fun () ->
          let g =
            Hedgehog.Gen.resize 10
              (Hedgehog.Gen.sized (fun s -> Hedgehog.Gen.return s))
          in
      ...
      check "pair generates pairs" (fun () ->
          let g =
            Hedgehog.Gen.pair (Hedgehog.Gen.return 1) (Hedgehog.Gen.return 2)
          in
          match g 30 seed with
...
let test_property () =
  group "Property" (fun () ->
      ...
      check "failing property reports failure" (fun () ->
          ...
          match report.status with Property.Failed _ -> true | _ -> false);

      check "=== passes for equal values" (fun () ->
          let open Hedgehog in
          let prop =
            Property.property Gen.(return (fun () -> Property.( === ) 42 42))
          in
          Property.check prop);
      ...
      check "=== fails for unequal values" (fun () ->
          let open Hedgehog in
          let prop =
            Property.property Gen.(return (fun () -> Property.( === ) 1 2))
          in
          let report = Property.check_report prop in
          match report.status with Property.Failed _ -> true | _ -> false);
      ...
      check "annotate is captured in log" (fun () ->
          ...
          | Property.Failed { log; _ } ->
              List.exists
                (function
                  | Property.Annotation s -> String.length s > 0 | _ -> false)
                log
          | _ -> false);
      ...
      check "discard leads to GaveUp" (fun () ->
          ...
          in
          let report = Property.check_report prop in
          match report.status with Property.GaveUp -> true | _ -> false);
      ...
      check "footnote is captured in log" (fun () ->
          ...
          | Property.Failed { log; _ } ->
              List.exists
                (function
                  | Property.Footnote s -> s = "extra info" | _ -> false)
                log
          | _ -> false);
          ...
          let prop =
            Property.property
              Gen.(
                ...
                return (fun () -> Property.assert_ (x + y >= 0)))
          in
          Property.check prop);
      ...
      check "with_discards affects give-up" (fun () ->
          let open Hedgehog in
          let prop =
            Property.property Gen.discard |> Property.with_discards 3
          in
          let report = Property.check_report prop in
          ...
          let prop =
            Property.property
              Gen.(
                ...
                return (fun () ->
                    Property.tripping string_of_int Fun.id string_of_int
                      int_of_string_opt n))
          in
          Property.check prop);
      ...
      check "tripping annotates on failure" (fun () ->
          ...
          | Property.Failed { log; _ } ->
              let annotations =
                List.filter_map
                  (function Property.Annotation s -> Some s | _ -> None)
                  log
              in
      ...
      check "eval_result fails on Error" (fun () ->
          ...
          let prop =
            Property.property
              Gen.(
                return (fun () ->
                    ignore (Property.eval_result Fun.id (Error "bad"))))
          in
          let report = Property.check_report prop in
          match report.status with
          | Property.Failed { failure; _ } -> failure.message = "bad"
          | _ -> false);
          ...
          let prop =
            Property.property
              Gen.(
                let* x = int (Range.constant 0 100) in
                return (fun () -> Property.assert_ (x >= 0)))
            |> Property.with_verbose |> Property.with_tests 10
          in
          let report = Property.check_report prop in
          report.tests = 10 && report.status = Property.OK);
          ...
          | Property.Failed { log; _ } ->
              List.exists
                (fun entry ->
                  match entry with
                  | Property.Annotation s -> (
                      try Scanf.sscanf s "n = %d" Fun.id = 500 with _ -> false)
                  | _ -> false)
                log
          | _ -> false);
      ...
      check "with_verbose give-up property" (fun () ->
          let open Hedgehog in
          let prop =
            Property.property Gen.discard
            |> Property.with_verbose |> Property.with_discards 3
          in
          let report = Property.check_report prop in
          report.status = Property.GaveUp && report.discards = 3);
          ...
          let prop =
            Property.property
              Gen.(
                let* x = int (Range.constant 0 100) in
                return (fun () -> Property.assert_ (x >= 0)))
            |> Property.with_tests 50 |> Property.with_shrinks 200
            |> Property.with_discards 10
          in
          let report = Property.check_report prop in
...
let test_gen_combinators () =
  group "Gen combinators" (fun () ->
      let seed = Hedgehog.Seed.from 42L in

      check "non_empty always produces length >= 1" (fun () ->
          let g =
            Hedgehog.Gen.non_empty
              (Hedgehog.Range.constant 0 5)
              (Hedgehog.Gen.return 1)
          in
          let results =
            Array.init 50 (fun i ->
      ...
      check "shuffle produces a permutation" (fun () ->
          ...
          let results =
            Array.init 50 (fun i ->
                let s = Hedgehog.Seed.from (Int64.of_int (i + 1)) in
                match g 30 s with Some t -> Hedgehog.Tree.value t | None -> [])
          in
          Array.for_all
            (fun ys ->
      ...
      check "either generates both Left and Right" (fun () ->
          let g =
            Hedgehog.Gen.either (Hedgehog.Gen.return 1) (Hedgehog.Gen.return 2)
          in
          let results =
            Array.init 50 (fun i ->
                let s = Hedgehog.Seed.from (Int64.of_int (i + 1)) in
          ...
          let has_left =
            Array.exists
              (fun v -> match v with Either.Left _ -> true | _ -> false)
              results
          in
          let has_right =
            Array.exists
              (fun v -> match v with Either.Right _ -> true | _ -> false)
              results
          in
          has_left && has_right);
      ...
      check "unique produces no duplicates" (fun () ->
          let g =
            Hedgehog.Gen.unique Int.compare
...
let test_coverage () =
  group "Coverage" (fun () ->
      check "label records all tests" (fun () ->
          let open Hedgehog in
          let prop =
            Property.property Gen.(return (fun () -> Property.label "always"))
          in
          let report = Property.check_report prop in
          match report.status with
          | Property.OK -> (
              match (report.coverage : Property.label_info list) with
              | [ { name = "always"; count; _ } ] -> count = report.tests
              | _ -> false)
          | _ -> false);
            ...
            Property.property
              Gen.(
                ...
                return (fun () ->
                    ...
                    Property.classify "odd" (x mod 2 <> 0)))
          in
          let report = Property.check_report prop in
          match report.status with
          | Property.OK ->
              let has_even =
                List.exists
                  (fun (li : Property.label_info) -> li.name = "even")
                  report.coverage
              in
              let has_odd =
                List.exists
                  (fun (li : Property.label_info) -> li.name = "odd")
                  report.coverage
              in
              has_even && has_odd
          | _ -> false);
          ...
          let prop =
            Property.property
              Gen.(
                ...
                return (fun () -> Property.cover 30.0 "positive" (x > 0)))
          in
          let report = Property.check_report prop in
          match report.status with Property.OK -> true | _ -> false);

      check "cover fails when not met" (fun () ->
          let open Hedgehog in
          let prop =
            Property.property
              Gen.(return (fun () -> Property.cover 99.0 "impossible" false))
          in
          let report = Property.check_report prop in
          match report.status with Property.Failed _ -> true | _ -> false);
      ...
      check "collect creates per-value labels" (fun () ->
          ...
          let prop =
            Property.property
              Gen.(
                let* b = bool in
                return (fun () -> Property.collect string_of_bool b))
          in
          let report = Property.check_report prop in
          match report.status with
          | Property.OK ->
              let has_true =
                List.exists
                  (fun (li : Property.label_info) -> li.name = "true")
                  report.coverage
              in
              let has_false =
                List.exists
                  (fun (li : Property.label_info) -> li.name = "false")
                  report.coverage
              in
              has_true && has_false
          | _ -> false);
      ...
      check "coverage failure produces Failed status with message" (fun () ->
          ...
          let prop =
            Property.property
              Gen.(return (fun () -> Property.cover 50.0 "always-false" false))
          in
          let report = Property.check_report prop in
          match report.status with
          | Property.Failed { failure; _ } ->
              ...
              let rec has_substring haystack needle i =
                ...
                else if String.sub haystack i (String.length needle) = needle
                then true
      ...
      check "coverage passes with OK status" (fun () ->
          ...
          let prop =
            Property.property
              Gen.(
                let* x = int (Range.constant 0 100) in
                return (fun () -> Property.cover 0.0 "anything" (x >= 0)))
          in
          let report = Property.check_report prop in
...
let test_subterm () =
  group "Subterm" (fun () ->
      ...
      check "subterm root is f(x)" (fun () ->
          let g =
            Hedgehog.Gen.subterm (Hedgehog.Gen.return 5) (fun x -> x + 100)
          in
          match g 30 seed with
      ...
      check "subterm2 first children are both subterms" (fun () ->
          let g =
            Hedgehog.Gen.subterm2 (Hedgehog.Gen.return 10)
              (Hedgehog.Gen.return 20) (fun x y -> x + y + 1000)
          in
          match g 30 seed with
          | Some t -> (
      ...
      check "subterm3 first children are all three subterms" (fun () ->
          let g =
            Hedgehog.Gen.subterm3 (Hedgehog.Gen.return 10)
              (Hedgehog.Gen.return 20) (Hedgehog.Gen.return 30) (fun x y z ->
                x + y + z + 1000)
          in
          match g 30 seed with
          | Some t -> (
      ...
      check "subterm_m correct root and subterm child" (fun () ->
          let g =
            Hedgehog.Gen.subterm_m (Hedgehog.Gen.return 10) (fun x ->
                Hedgehog.Gen.return (x + 1))
          in
          match g 30 seed with
          | Some t -> (
              ...
              Gen.(
                ...
                return (fun () ->
                    Property.annotate
                      ...
                      | Add _ -> "Add");
                    match e with Neg _ -> Property.assert_ false | _ -> ()))
          in
          let report = Property.check_report prop in
          match report.status with
          | Property.Failed { log; _ } ->
              ...
              List.exists
                (function Property.Annotation "Neg" -> true | _ -> false)
                log
          | _ ->
          ...
          let prop =
            Property.property
              Gen.(
                let* e = gen_expr in
                return (fun () ->
                    match e with Add _ -> Property.assert_ false | _ -> ()))
          in
          let report = Property.check_report prop in
          match report.status with
          | Property.Failed { log; _ } ->
              ...
              let has_annotation =
                List.exists
                  (function Property.Annotation _ -> true | _ -> false)
                  log
              in
...
module Counter_spec = struct
  ...
  let next_state cmd state =
    match cmd with Incr -> state + 1 | Decr -> state - 1 | Get -> state
...
let test_stm () =
  group "Stm" (fun () ->
      ...
      check "sequential: buggy counter fails" (fun () ->
          let report =
            Hedgehog.Property.check_report (Buggy_stm.sequential ())
          in
          match report.status with
      ...
      check "sequential: buggy counter shrinks to minimal" (fun () ->
          ...
          | Hedgehog.Property.Failed { log; _ } ->
              ...
              let annotations =
                List.filter_map
                  (function
                    | Hedgehog.Property.Annotation s -> Some s | _ -> None)
                  log
              in
...
let test_diff () =
  group "Diff" (fun () ->
      check "identical inputs produce all Same" (fun () ->
          ...
          List.for_all
            (fun e -> match e with Hedgehog.Diff.Same _ -> true | _ -> false)
            d.edits
      ...
      check "completely different produces Removed + Added" (fun () ->
          ...
          let has_removed =
            List.exists
              (fun e ->
                match e with Hedgehog.Diff.Removed "aaa" -> true | _ -> false)
              d.edits
          in
          let has_added =
            List.exists
              (fun e ->
                match e with Hedgehog.Diff.Added "bbb" -> true | _ -> false)
              d.edits
          in
      ...
      check "partial overlap produces correct interleaving" (fun () ->
          ...
          match d.edits with
          | [
           Hedgehog.Diff.Same "a";
           Hedgehog.Diff.Removed "b";
           Hedgehog.Diff.Added "x";
           Hedgehog.Diff.Same "c";
          ] ->
              true
          | _ -> false);
      ...
      check "of_strings splits on newlines correctly" (fun () ->
          let d =
            Hedgehog.Diff.of_strings "line1\nline2\nline3" "line1\nline2\nline3"
          in
          List.length d.edits = 3
          && List.for_all
               (fun e ->
                 match e with Hedgehog.Diff.Same _ -> true | _ -> false)
               d.edits);
      ...
      check "empty string handling" (fun () ->
          ...
          let has_added =
            List.exists
              (fun e ->
                match e with Hedgehog.Diff.Added "hello" -> true | _ -> false)
              d.edits
          in
          ...
          let has_removed =
            List.exists
              (fun e ->
                match e with
                | Hedgehog.Diff.Removed "hello" -> true
                | _ -> false)
              d2.edits
          in
...
let test_color_rendering () =
  group "Color rendering" (fun () ->
      check "diff assertion produces failure.diff = Some" (fun () ->
          ...
          let prop =
            Property.property
              Gen.(
                return (fun () ->
                    Property.diff string_of_int ( = ) string_of_int 1 2))
          in
          let report = Property.check_report prop in
          match report.status with
          | Property.Failed { failure; _ } -> (
      ...
      check "format_report ~color:false with diff has - / + lines without ANSI"
        (fun () ->
          let open Hedgehog in
          let prop =
            Property.property
              Gen.(
                return (fun () ->
                    Property.diff Fun.id ( = ) Fun.id "old_value" "new_value"))
          in
          let report = Property.check_report prop in
          ...
          let has_minus =
            let rec f i =
              ...
              else if String.sub s i 2 = "- " then true
              else f (i + 1)
            in
            f 0
          in
      ...
      check "format_report without ~color on OK/Failed/GaveUp has no ANSI codes"
        (fun () ->
          let open Hedgehog in
          (* OK *)
          let prop_ok = Property.property Gen.(return (fun () -> ())) in
          let report_ok = Property.check_report prop_ok in
          let s_ok = Property.format_report report_ok in
          (* Failed *)
          let prop_fail =
            Property.property Gen.(return (fun () -> Property.assert_ false))
          in
