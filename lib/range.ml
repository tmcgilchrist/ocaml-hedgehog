...
let singleton x =
  { origin = x; bounds = fun _ -> (x, x) }

let constant_from z x y =
  { origin = z; bounds = fun _ -> (x, y) }
...
let scale_exponential sz z n =
  Float.to_int (Float.round (scale_exponential_float sz (float_of_int z) (float_of_int n)))
