...
let return x _size _seed = Some (Tree.singleton x)
...
let map f m size seed =
  match m size seed with None -> None | Some tree -> Some (Tree.map f tree)
...
let generate f size seed = Some (Tree.singleton (f size seed))

(* -- Size control -- *)

let sized f size seed = f size size seed
let resize new_size gen _size seed = gen new_size seed
...
let golden x = Float.to_int (Float.round (float_of_int x *. 0.61803398875))
let small gen = scale golden gen
...
let bool _size seed =
  ...
  if n = 0 then Some (Tree.singleton false)
  else
    Some
      (Tree.Node (true, fun () -> Seq.Cons (Tree.singleton false, Seq.empty)))

(* -- Character generators -- *)

let char range size seed = map Char.chr (integral range) size seed
...
let alpha size seed =
  ...
  if n = 0 then lower size seed' else upper size seed'
...
let frequency weighted_gens size seed =
  ...
  | _ ->
      ...
      let rec pick remaining = function
        ...
        | (w, gen) :: rest ->
            if remaining <= w then gen else pick (remaining - w) rest
      in
...
let recursive f nonrec_gens rec_gens =
  sized (fun size ->
      if size <= 1 then f nonrec_gens
      else f (nonrec_gens @ List.map small rec_gens))
...
let option gen size seed =
  ...
  if n = 0 then Some (Tree.singleton None)
  else map (fun x -> Some x) gen size seed'
...
let ensure p gen size seed =
  match gen size seed with
  | None -> None
  | Some tree -> if p (Tree.value tree) then Some tree else None
...
let filter p gen size seed =
  let rec try_gen k size' seed' =
    if k > 100 then None
      ...
      | Some tree ->
          if p (Tree.value tree) then Some (Tree.filter p tree)
          else try_gen (k + 1) size' s2
  in
  try_gen 0 size seed
...
let freeze gen size seed =
  ...
  | Some tree ->
      Some
        (Tree.map (fun x -> (x, fun _sz _sd -> Some (Tree.singleton x))) tree)
...
let int32 range size seed =
  ...
  if n = int_origin then Some (Tree.singleton (Int32.of_int n))
    ...
    let tree =
      Tree.unfold Int32.of_int (fun x -> Shrink.towards int_origin x) n
    in
          ...
          ( f x y z,
            Seq.append
              (fun () ->
                Seq.Cons
                  ( tx,
                    fun () -> Seq.Cons (ty, fun () -> Seq.Cons (tz, Seq.empty))
                  ))
              (Seq.append
            ...
            Some
              (Tree.Node
                 ( r,
                   Seq.append
                     (fun () ->
                       Seq.Cons (tx, fun () -> Seq.Cons (ty, Seq.empty)))
                     (Seq.append
                        (Seq.append
...
let sample ?(size = 30) ?seed gen =
  let seed = match seed with Some s -> s | None -> Seed.random () in
  let rec loop n seed =
