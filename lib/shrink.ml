...
let cons_nub x = function
  | y :: _ as ys when x = y -> ys
  | ys -> x :: ys

let halves n =
  let rec go n () =
    if n = 0 then Seq.Nil
    else Seq.Cons (n, go (n / 2))
  in
  go n
...
let towards_float destination x =
  if destination = x then
    ...
    let rec iterate_halve d () =
      let v = x -. d in
      if v = x || Float.is_nan v || Float.is_infinite v then
        Seq.Nil
      else
        Seq.Cons (v, iterate_halve (d /. 2.0))
    in
    iterate_halve diff
