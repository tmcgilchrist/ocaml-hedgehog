...
and fold_forest f g xs =
  g (Seq.map (fold f g) xs)

let rec unfold f g x =
  Node (f x, unfold_forest f g x)
and unfold_forest f g x =
  Seq.map (unfold f g) (g x)
...
let rec prune n (Node (x, xs)) =
  if n <= 0 then
    ...
    Node (x, Seq.map (prune (n - 1)) xs)

let rec filter p (Node (x, xs)) =
  Node (x, filter_forest p xs)
and filter_forest p xs =
  xs
  |> Seq.filter (fun t -> p (value t))
  |> Seq.map (filter p)
...
let rec map_option p (Node (x, xs)) =
  ...
  | None ->
    ...
    (match passing () with
     | Seq.Nil -> None
     | Seq.Cons (Node (y, ys0), rest) ->
       Some (Node (y, Seq.append ys0 rest)))
  | Some y ->
    Some (Node (y, Seq.filter_map (map_option p) xs))
...
let cons_child a (Node (x, xs)) =
  Node (x, fun () -> Seq.Cons (singleton a, xs))
...
let removes k xs =
  let rec go = function
    | [] -> []
    | xs ->
      let xs1, xs2 = List.filteri (fun i _ -> i < k) xs,
                      List.filteri (fun i _ -> i >= k) xs in
...
let to_node (Node (x, xs)) =
  { nvalue = x; nchildren = List.of_seq xs }
...
let rec lazy_concat_map f xs () =
  ...
  | [] -> Seq.Nil
  | x :: rest ->
    lazy_append_seq (List.to_seq (f x)) (lazy_concat_map f rest) ()
