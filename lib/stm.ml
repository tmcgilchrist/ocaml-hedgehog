...
module Make (S : Spec) = struct
    ...
    let rec go state = function
      ...
      | cmd :: rest ->
        if S.precond state cmd then
          go (S.next_state cmd state) rest
        else false
    in
  ...
  let shrink_cmds cmds =
    ...
    let removals =
      let halves n =
        let rec go n = if n <= 0 then Seq.empty
          else fun () -> Seq.Cons (n, go (n / 2))
        in go n
      in
      Seq.flat_map (fun k ->
        let rec removes k = function
          ...
          | xs ->
            ...
            fun () -> Seq.Cons (xs2,
              Seq.map (fun rest -> xs1 @ rest) (removes k xs2))
        in
        removes k cmds
      ) (halves len)
    in
    ...
    let singles =
      let rec go prefix = function
        | [] -> Seq.empty
        | cmd :: suffix ->
          let shrunk = Seq.map (fun cmd' ->
            List.rev_append prefix (cmd' :: suffix)
          ) (S.shrink_cmd cmd) in
          Seq.append shrunk (go (cmd :: prefix) suffix)
      in
      go [] cmds
    in
    Seq.append removals singles
    |> Seq.filter preconds_valid

  (* Build a shrink tree for a command list. *)
  let cmd_tree cmds =
    Tree.unfold Fun.id (fun cs -> shrink_cmds cs) cmds

  (* Execute a command sequence on a SUT, checking postconditions. *)
  let run_sequential cmds =
    let sut = S.init_sut () in
    Fun.protect ~finally:(fun () -> S.cleanup sut) (fun () ->
      let rec go state = function
        | [] -> ()
        | cmd :: rest ->
          let result = S.run cmd sut in
          Property.annotate (Printf.sprintf "%s => %s"
            (S.show_cmd cmd) (S.show_result result));
          if not (S.postcond cmd state result) then
            Property.failure ()
          else
            go (S.next_state cmd state) rest
      in
  ...
  let valid_interleaving state trace =
    let rec go state = function
      | [] -> true
      | (cmd, result) :: rest ->
        if S.postcond cmd state result then
          go (S.next_state cmd state) rest
        else false
    in
    go state trace
  ...
  let par_tree triple =
    Tree.unfold Fun.id (fun t -> shrink_par_triple t) triple

  let show_trace trace =
    String.concat "; " (List.map (fun (cmd, result) ->
  ...
  let run_parallel (prefix, branch1, branch2) () =
    ...
    Fun.protect ~finally:(fun () -> S.cleanup sut) (fun () ->
      ...
      let state_after_prefix =
        List.fold_left (fun state cmd ->
          ...
          if not (S.postcond cmd state result) then begin
            Property.annotate (Printf.sprintf "Prefix failed: %s => %s"
              (S.show_cmd cmd) (S.show_result result));
            Property.failure ()
          end;
      ...
      if not (is_linearizable state_after_prefix !trace1 !trace2) then begin
        Property.annotate (Printf.sprintf "Prefix: [%s]"
          (String.concat "; " (List.map S.show_cmd prefix)));
        Property.annotate (Printf.sprintf "Branch 1: [%s]" (show_trace !trace1));
        Property.annotate (Printf.sprintf "Branch 2: [%s]" (show_trace !trace2));
        Property.failure ()
      end)
